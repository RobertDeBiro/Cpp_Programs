...
2. [The Problem: Duplicated Algorithms](#the-problem-duplicated-algorithms)
3. [Real-World Example: Robot Initialization Sequence](#real-world-example-robot-initialization-sequence)
4. [Design Principles](#design-principles)
5. [The Solution: Template Method Pattern](#the-solution-template-method-pattern)
...
## Real-World Example: Robot Initialization Sequence

### Scenario: Autonomous Mobile Robot Startup

You are developing **firmware for autonomous mobile robots** used in warehouses. Different robot models (AMR-1000, AMR-2000, AMR-3000) share the same startup procedure but have different hardware components:

**Common Startup Sequence:**
1. **Power-On Self-Test (POST)** - Check system integrity
2. **Initialize Communication** - Connect to base station
3. **Calibrate Sensors** - Hardware-specific calibration
4. **Initialize Actuators** - Hardware-specific motor initialization
5. **Load Navigation Map** - Common for all robots
6. **Self-Test Complete** - Optional hook for extended diagnostics
7. **Ready Signal** - Indicate ready for operation

**Problem:** Each robot model has different sensors and actuators, but the startup sequence must be identical for system consistency.

### Bad Architecture: Duplicated Initialization Algorithm

```cpp
// BAD: Each robot model duplicates the entire initialization sequence
class AMR1000Robot {
private:
    LaserScanner laser;
...
public:
    void startup() {
        // POST
        std::cout << "AMR-1000: Running Power-On Self-Test...\n";
        if (!checkSystemIntegrity()) {
            throw std::runtime_error("POST failed");
        }

        // Communication
        std::cout << "AMR-1000: Initializing communication...\n";
        initWiFi();
        ...
        // Load map
        std::cout << "AMR-1000: Loading navigation map...\n";
        loadMapFromBaseStation();

        // Ready
        std::cout << "AMR-1000: Ready for operation\n";
        setStatus(READY);
    }

    bool checkSystemIntegrity() { /* ... */ return true; }
    void initWiFi() { /* ... */ }
    void connectToBaseStation() { /* ... */ }
    void loadMapFromBaseStation() { /* ... */ }
...
public:
    void startup() {
        // POST - DUPLICATED ALGORITHM
        std::cout << "AMR-2000: Running Power-On Self-Test...\n";
        if (!checkSystemIntegrity()) {
            throw std::runtime_error("POST failed");
        }

        // Communication - DUPLICATED ALGORITHM
        std::cout << "AMR-2000: Initializing communication...\n";
        initWiFi();
        connectToBaseStation();

        // Calibrate sensors - DIFFERENT HARDWARE
        std::cout << "AMR-2000: Calibrating 3D LIDAR...\n";
        lidar.calibrate();
        lidar.performSelfTest();

        std::cout << "AMR-2000: Calibrating vision camera...\n";
        camera.calibrate();
        camera.performWhiteBalance();

        // Initialize actuators - DIFFERENT HARDWARE
        std::cout << "AMR-2000: Initializing omni-directional drive...\n";
        drive.enableMotors();
        drive.calibrateWheels();

        // Load map - DUPLICATED ALGORITHM
        std::cout << "AMR-2000: Loading navigation map...\n";
        loadMapFromBaseStation();

        // Ready - DUPLICATED ALGORITHM
        std::cout << "AMR-2000: Ready for operation\n";
        setStatus(READY);
    }

    bool checkSystemIntegrity() { /* DUPLICATE */ return true; }
    void initWiFi() { /* DUPLICATE */ }
    void connectToBaseStation() { /* DUPLICATE */ }
    void loadMapFromBaseStation() { /* DUPLICATE */ }
    void setStatus(int status) { /* DUPLICATE */ }
};

class AMR3000Robot {
private:
    MultiSensorArray sensors;
    MecanumDrive drive;
    RoboticArm arm;

public:
    void startup() {
        // POST - DUPLICATED ALGORITHM (3rd time!)
        std::cout << "AMR-3000: Running Power-On Self-Test...\n";
        if (!checkSystemIntegrity()) {
            throw std::runtime_error("POST failed");
        }

        // Communication - DUPLICATED ALGORITHM (3rd time!)
        std::cout << "AMR-3000: Initializing communication...\n";
        initWiFi();
        connectToBaseStation();
        ...
        std::cout << "AMR-3000: Initializing robotic arm...\n";
        arm.enableMotors();
        arm.moveToHomePosition();

        // Load map - DUPLICATED ALGORITHM (3rd time!)
        std::cout << "AMR-3000: Loading navigation map...\n";
        loadMapFromBaseStation();

        // Ready - DUPLICATED ALGORITHM (3rd time!)
        std::cout << "AMR-3000: Ready for operation\n";
        setStatus(READY);
    }

    bool checkSystemIntegrity() { /* DUPLICATE 3 */ return true; }
    void initWiFi() { /* DUPLICATE 3 */ }
    void connectToBaseStation() { /* DUPLICATE 3 */ }
    void loadMapFromBaseStation() { /* DUPLICATE 3 */ }
    void setStatus(int status) { /* DUPLICATE 3 */ }
};
```

### Why This Fails in Production

1. **Algorithm Duplication Disaster**
   - Startup sequence duplicated across 3 robot classes
   - POST, communication, map loading duplicated 3 times
   - Total: ~90 lines of duplicated algorithm code
   - Adding 4th robot model? Duplicate again!

2. **Impossible to Modify Startup Sequence**
   - Fleet management wants to add "battery health check" before POST
   - Must modify AMR1000, AMR2000, AMR3000, and every future robot
   - Developer modifies 2 out of 3 classes, forgets AMR-3000
   - Result: Inconsistent startup behavior across fleet

3. **Safety-Critical Bug Example**
   ```cpp
   // Critical bug discovered: Must verify emergency stop before enabling motors

   // Fixed in AMR-1000
   void startup() {
       // ... POST, communication, calibrate sensors ...
       verifyEmergencyStop();  // ✓ Added
       drive.enableMotors();
       // ...
   }
   ...
   // ❌ BUG: Forgot to update AMR-3000!
   void startup() {
       // ... POST, communication, calibrate sensors ...
       // ❌ MISSING: verifyEmergencyStop()
       drive.enableMotors();  // DANGER: Motors enabled without E-stop check!
       // ...
   }

   // RESULT: AMR-3000 robot starts moving with broken emergency stop
   // Safety incident: Robot collides with worker
   // Root cause: Forgot to update one of three duplicated algorithms
   // 8 hours debugging, $50K in damages
   ```

4. **Maintenance Nightmare**
   - Algorithm scattered across 3 classes
   - Each class has 8 duplicate methods
   - Bug in `connectToBaseStation()`? Fix it 3 times
   - Forgot one? Production incident

5. **Cannot Enforce Startup Order**
   - Nothing prevents developer from changing order in one class:
   ```cpp
   // AMR-3000 developer accidentally changes order
   void startup() {
       initWiFi();              // ❌ Wrong order!
       calibrateSensors();
       checkSystemIntegrity();  // POST should be FIRST!
       // ... crashes because sensors calibrated before POST
   }
   ```

6. **New Robot Models Compound the Problem**
   ```cpp
   // Adding AMR-4000: Must duplicate entire algorithm AGAIN (4th time)
   class AMR4000Robot {
       void startup() {
           // Copy-paste 40 lines from AMR-3000
           // Change sensor/actuator calls
...
```cpp
// Fleet management system update: Add telemetry logging during startup
// Developer updates only AMR-1000 and AMR-2000, misses AMR-3000

// AMR-1000 startup (updated)
void startup() {
    logTelemetry("POST_START");
    checkSystemIntegrity();
    logTelemetry("POST_COMPLETE");
    // ... rest of startup with telemetry
}

// AMR-2000 startup (updated)
void startup() {
    logTelemetry("POST_START");
    checkSystemIntegrity();
    logTelemetry("POST_COMPLETE");
...
// AMR-3000 startup (❌ NOT UPDATED)
void startup() {
    checkSystemIntegrity();  // No telemetry!
    // ... rest of startup without telemetry
}

// PRODUCTION ISSUE:
// 1. Fleet monitoring dashboard shows AMR-1000 and AMR-2000 startups
// 2. AMR-3000 robots "disappear" during startup (no telemetry)
// 3. Operations team thinks AMR-3000 units are failing
// 4. Dispatch technicians to check robots (cost: $2000)
// 5. Robots are fine, just missing telemetry code
// 6. Root cause: Forgot to update third duplicate
// 7. Debug time: 4 hours, false alarm cost: $2000
```

### Another Failure: Optional Diagnostics

```cpp
// Want to add OPTIONAL extended diagnostics for AMR-2000 only
// (AMR-2000 has advanced sensors that need extra checks)

class AMR2000Robot {
    void startup() {
        // ... standard startup ...

        // Want optional extended diagnostics here
        // But algorithm is hardcoded!
        // Cannot make it conditional without modifying entire method

        performExtendedDiagnostics();  // Always runs, no way to skip

        // ... rest of startup ...
    }

    void performExtendedDiagnostics() {
        // Takes 30 seconds
        // Sometimes we want to skip this for fast startup
        // But cannot - algorithm is rigid!
    }
};

// PROBLEM: Cannot make parts of algorithm optional
// No "hooks" for subclass-specific behavior
// Algorithm is all-or-nothing
```
...
### 1. Non-Virtual Template Method (NVI - Non-Virtual Interface Idiom)

In C++, template method should be **non-virtual** to prevent subclasses from overriding algorithm structure.

```cpp
// GOOD: Template method is non-virtual (cannot override)
class CaffeineBeverage {
public:
    void prepareRecipe() {  // Non-virtual - NVI idiom
        boilWater();
        brew();
        pourInCup();
        addCondiments();
...
protected:
    virtual void brew() = 0;           // Virtual: subclass overrides
    virtual void addCondiments() = 0;   // Virtual: subclass overrides
};

// Coffee cannot override prepareRecipe() - algorithm protected
class Coffee : public CaffeineBeverage {
protected:
    void brew() override { /* ... */ }
    void addCondiments() override { /* ... */ }
    // Cannot override prepareRecipe() - it's non-virtual
};
```

**Why NVI?**

- Protects algorithm invariant
- Base class controls pre/post conditions
- Subclasses only override specific steps
- Clear contract: public interface (non-virtual) vs protected customization (virtual)

**Bad practice (Java style in C++):**

```cpp
// BAD in C++: Template method is virtual
class CaffeineBeverage {
public:
    virtual void prepareRecipe() {  // ❌ Virtual - can be overridden!
        boilWater();
        brew();
        pourInCup();
        addCondiments();
...
// ❌ Subclass can break algorithm structure
class Coffee : public CaffeineBeverage {
public:
    void prepareRecipe() override {  // ❌ Overriding algorithm!
        brew();
        boilWater();  // Wrong order!
        // ... algorithm broken
    }
};
```
...
Always provide virtual destructor in base class with virtual methods.

```cpp
class CaffeineBeverage {
public:
    virtual ~CaffeineBeverage() = default;  // Virtual destructor
...
delete beverage;  // Calls Coffee destructor, then CaffeineBeverage destructor
```

### 3. Protected vs Private Virtual Methods

**Protected virtual:** Subclass can call base implementation

```cpp
class CaffeineBeverage {
protected:
    virtual void brew() {
        std::cout << "Default brewing\n";
    }
};

class Coffee : public CaffeineBeverage {
    void brew() override {
        CaffeineBeverage::brew();  // Call base implementation
        std::cout << "Coffee-specific brewing\n";
    }
};
```

**Private virtual:** Subclass cannot call base implementation (stricter encapsulation)

```cpp
class CaffeineBeverage {
public:
    void prepareRecipe() {
        brew();  // Call private virtual
    }

private:
    virtual void brew() = 0;  // Private: subclass cannot call base
};

class Coffee : public CaffeineBeverage {
private:
    void brew() override {
        // Cannot call CaffeineBeverage::brew()
        std::cout << "Coffee brewing\n";
    }
};
```

**Choose protected for:** Allowing subclass to extend base behavior

**Choose private for:** Enforcing complete replacement of behavior

### 4. Final Keyword (C++11+)

Prevent further overriding of specific methods.
...
class Coffee : public CaffeineBeverage {
protected:
    void brew() override final {  // Cannot be overridden further
        std::cout << "Coffee brewing\n";
    }
...
class SpecialtyCoffee : public Coffee {
    // Cannot override brew() - it's final in Coffee
    // void brew() override { }  // ❌ Compilation error

    void addCondiments() override {  // ✓ OK - not final
        std::cout << "Adding special condiments\n";
    }
};
```
...
protected:
    ...
    // Regular virtual: Can override, but not required
    virtual bool customerWantsCondiments() {
...
// CaffeineBeverage beverage;  // ❌ Error: abstract class

// Must create concrete subclass
class Coffee : public CaffeineBeverage {
    void brew() override { /* implement */ }
    void addCondiments() override { /* implement */ }
    // customerWantsCondiments() optional - has default
};

Coffee coffee;  // ✓ OK - all pure virtuals implemented
```

### 6. Smart Pointers with Polymorphism
...
void prepareBeverages() {
    ...
    std::vector<std::unique_ptr<CaffeineBeverage>> beverages;

    beverages.push_back(std::make_unique<Coffee>());
    beverages.push_back(std::make_unique<Tea>());
    beverages.push_back(std::make_unique<HotChocolate>());

    for (const auto& beverage : beverages) {
        beverage->prepareRecipe();  // Polymorphic call
    }
...
### 7. Template Method with CRTP (Curiously Recurring Template Pattern)

Alternative to virtual functions for compile-time polymorphism (zero runtime overhead).

```cpp
// CRTP version - no virtual functions, no runtime overhead
template<typename Derived>
class CaffeineBeverage {
public:
    void prepareRecipe() {
        boilWater();
        static_cast<Derived*>(this)->brew();
        pourInCup();
        static_cast<Derived*>(this)->addCondiments();
    }
...
public:
    ...
    void addCondiments() { std::cout << "Adding milk\n"; }
};

class Tea : public CaffeineBeverage<Tea> {
public:
    void brew() { std::cout << "Steeping tea\n"; }
    void addCondiments() { std::cout << "Adding lemon\n"; }
};

// Usage
Coffee coffee;
coffee.prepareRecipe();  // No virtual function overhead!

// Trade-off: Cannot store in heterogeneous container
// std::vector<CaffeineBeverage*> beverages;  // ❌ Cannot do this with CRTP
```

**CRTP Pros:**
- Zero runtime overhead (no vtable)
- Compile-time polymorphism
- Same template method pattern structure

**CRTP Cons:**
- Cannot use polymorphically (no base class pointer)
- More complex template syntax
- Longer compile times

**When to use CRTP:** Performance-critical code where polymorphism not needed at runtime.
...
class CaffeineBeverage {
public:
    void prepareRecipe() {  // Non-const: modifies state
        boilWater();
        brew();
        pourInCup();
        addCondiments();
    ...
    int getBrewingTemperature() const {  // Const: query operation
        return brewingTemp;
    }
...
### 9. Exception Safety in Template Methods
...
public:
    void prepareRecipe() {
        try {
            ...
            cleanup();  // Ensure cleanup on exception
            throw;      // Re-throw
        }
    }
