...
## 2. Real-World Example: Robotics Motion Control 🤖

**Scenario:** You're building a mobile robot platform that needs different locomotion strategies:
...
class FlyingRobot : public Robot {
    void move() override {
        std::cout << "Flying with propellers\n";
    }
};

class WalkingRobot : public Robot {
    void move() override {
        std::cout << "Walking on legs\n";
    }
};
```
...
1. **Field Reconfiguration**: Robot hardware can change in the field (wheels → legs upgrade). With inheritance, you'd need to **destroy and recreate the entire robot object**!

2. **Hybrid Locomotion**: Mars rovers might walk on rough terrain but drive on flat surfaces. Can't combine behaviors!

3. **Emergency Modes**: If a wheel breaks, switch to "limp mode" (3-wheel drive). Impossible with fixed inheritance!

4. **Testing**: Can't test wheel logic without instantiating a full `WheeledRobot` with all its dependencies.

**This is exactly the pain the Strategy pattern solves!**
...
**Robotics Example (continued):**

With Strategy pattern:

```cpp
class Robot {
    std::unique_ptr<LocomotionStrategy> locomotion;
...
public:
    ...
    // Field upgrade: wheels → legs
    void upgradeLocomotion(std::unique_ptr<LocomotionStrategy> newStrategy) {
        locomotion = std::move(newStrategy);
        // Robot hardware unchanged, behavior swapped!
    }
};

// In the field:
robot.move();  // Rolling on wheels
// Hardware upgrade installed
robot.upgradeLocomotion(std::make_unique<LegsLocomotion>());
robot.move();  // Walking on legs
```

**Key Production Benefits:**

- **Hot-swappable behaviors** (no system restart)
- **A/B testing** different strategies in production
- **Fallback strategies** (primary fails → switch to backup)
- **Telemetry per strategy** (which strategy is performing best?)
...
| Aspect | Java | C++ |
|--------|------|-----|
| **Interfaces** | `interface FlyBehavior` | Abstract class with pure virtual |
| **Memory Management** | Garbage collected | Manual (use smart pointers!) |
| **Polymorphism** | All methods virtual by default | Explicit `virtual` keyword |
| **Object Creation** | `new` everywhere | Smart pointers (`std::make_unique`) |

### Modern C++ Advantages

#### 1. **Smart Pointers = No Memory Leaks**

```cpp
class Duck {
    std::unique_ptr<FlyBehavior> flyBehavior;  // Auto cleanup!

public:
    void setFlyBehavior(std::unique_ptr<FlyBehavior> fb) {
        flyBehavior = std::move(fb);  // Transfer ownership
    }
};
```
...
- Duck **exclusively owns** its behaviors
- No sharing needed → no reference counting overhead
- **RAII:** Automatic cleanup when duck is destroyed

#### 2. **`std::function` Alternative (C++11+)**

For simple strategies, skip classes entirely:
...
public:
    Duck() {
        flyBehavior = []() { std::cout << "Flying!\n"; };
    }

    void performFly() { flyBehavior(); }

    void setFlyBehavior(std::function<void()> fb) {
        flyBehavior = std::move(fb);
    }
};

// Usage:
duck.setFlyBehavior([]() { std::cout << "Rocket flight!\n"; });
```

**Trade-offs:**

- ✅ Less boilerplate (no strategy classes)
- ✅ Great for simple behaviors
- ❌ Lose type safety (all lambdas have same signature)
- ❌ Harder to compose complex strategies
...
class FlyBehavior {
public:
    virtual void fly() const = 0;  // Mark const - doesn't modify behavior
...
public:
    void performFly() const {  // Can call on const duck
        flyBehavior->fly();
    }
};
```
...
void Duck::setFlyBehavior(std::unique_ptr<FlyBehavior> fb) {
    flyBehavior = std::move(fb);  // No copying, just transfer ownership
}

// Usage:
duck.setFlyBehavior(std::make_unique<FlyRocketPowered>());
// Behavior constructed in-place, moved into duck
```

#### 5. **Virtual Destructor (Critical in C++!)**

```cpp
class FlyBehavior {
public:
    virtual void fly() const = 0;
    virtual ~FlyBehavior() = default;  // MUST be virtual!
};
```
...
delete fb;  // Without virtual destructor: memory leak!
            // Only ~FlyBehavior() called, not ~FlyWithWings()
```
...
class PacketProcessor {
    std::unique_ptr<CompressionStrategy> compression;
...
// Strategies: NoCompression, GzipCompression, LZ4Compression
// Switch based on network conditions
```
...
class AutonomousVehicle {
    std::unique_ptr<PathPlanningStrategy> planner;

public:
    void navigate() {
        Path path = planner->computePath(currentPos, goal);
        // follow path...
    }
};

// Strategies: AStarPlanner, RRTPlanner, DijkstraPlanner
// Switch based on environment complexity
```
...
public:
    SensorData process(RawData data) {
        return filter->apply(data);
    }
};

// Strategies: KalmanFilter, ParticleFilter, ComplementaryFilter
// Switch based on sensor noise characteristics
```
